...
import net.runelite.client.plugins.microbot.joatalcher.AreaDump;
    ...
    public boolean run() {
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if(getInitialPlayerLocation().isInArea(AreaDump.GE)) {
                    log.info("Player is in the area, starting script");
                } else {
                    log.info("Player is not in the area, waiting...");
                    sleep(5000);
                    return;
                }
/*
